...
```json
{
  "id": "instruction-example",
  "name": "示例指令",
  "file": "instructions/instruction-example.md",
  "origin": "local",
...
旧格式 profile 的 ID 在迁移时继续沿用，避免破坏已有任务状态。新建条目使用随机稳定 ID；ID 不显示在界面中。
...
4. 旧格式迁移只生成对应的指令项，不自动创建组合预设。
5. 全新安装创建六条可编辑默认指令，配置预设保持为空。
6. 用户库已有内容时只补充缺失的必要元数据，现有名称、正文、预设和任务状态保持原样。
...
设置窗口使用独立模态窗口，默认尺寸为 `960 × 640`，包含三个页签：
...
**设置**：集中选择界面语言和浅色/深色主题，显示当前数据目录并提供打开目录操作。应用后由主窗沿用现有主题过渡和语言自重启流程。

窗口工具栏保留一个主要入口 `导入包…`。指令页提供 `导出所选指令`，配置预设页提供 `导出配置预设`，更多菜单提供 `备份整个指令库` 和 `恢复备份`。空状态提供明确动作：`新增指令项`、`创建配置预设`、`导入包…`。
